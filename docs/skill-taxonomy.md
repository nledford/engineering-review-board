...
| --- | --- | --- |
| `adversarial-review` | Independent final challenge of completed changes, agent claims, merge readiness, hidden regressions, and unsupported assumptions. | Add. Provides a skeptical verification pass after implementation or primary review while reusing the general code-review evidence protocol. |
...
| `architecture-review` | Architecture-focused audits of boundaries, dependency direction, ports/adapters, Clean/Hexagonal/Onion/DDD hybrids, and modular structure. | Add. Separates review evidence and findings from architecture design and implementation guidance. |
...
| `domain-modeling` | Tactical DDD review of aggregates, entities, value objects, invariants, services, lifecycle ownership, and ubiquitous language. | Add. Provides a focused review lens while `domain-driven-design` owns model discovery and implementation. |
...
| `prompt-engineering-review` | Review and rewrite of coding-agent prompts, roles, delegation instructions, acceptance criteria, and quality gates. | Add. Covers prompt behavior and clarity while `create-agent-skill` owns reusable skill contracts. |
...
| `release-readiness` | Final merge or release assessment across acceptance criteria, tests, docs, migrations, rollout, rollback, operations, and cross-functional risk. | Add. Owns evidence-based ship or hold decisions after implementation and focused reviews are complete. |
...
| `testing-strategy` | Risk-focused review of test plans and suites, confidence gaps, flaky tests, boundary coverage, test levels, and maintainability. | Add. Separates test-strategy assessment from writing tests or running a TDD implementation loop. |
